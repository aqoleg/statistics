...
class ViewDetailed0 extends ViewDetailed {
    // canvas objects
    ...
    private void findYMinMax() {
        ...
        if (chart[0] == chart[1]) {
            ...
            yMax = yDataMax[0] > yDataMax[1] ? yDataMax[0] : yDataMax[1];
        } else {
            yMin = chart[0] ? yDataMin[0] : yDataMin[1];
            yMax = chart[0] ? yDataMax[0] : yDataMax[1];
        }
        yInterval = yMax - yMin;
    }
